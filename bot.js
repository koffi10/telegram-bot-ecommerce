...
// Maintenant, vous pouvez accéder à la variable
...
const initializeBot = async () => {
  try {
    ...
    // Si aucune donnée, créer les données par défaut
...
const getInlineKeyboard = (type, data = {}) => {
  ...
  return keyboards[type] || keyboards.categories;
};
...
bot.on('message', async (msg) => {
  if (msg.text && !msg.text.startsWith('/')) {
    ...
    switch (msg.text) {
      ...
      default:
        // Support intelligent - transférer à l'admin
...
const addToCart = async (chatId, userId, productId) => {
  ...
  if (user.cart[productId] > product.stock) {
    user.cart[productId] = product.stock;
